package com.example.k2p.domain.usecase

import com.example.k2p.core.exception.Failure
import com.example.k2p.core.functional.Either
import com.example.k2p.core.interactor.UseCase
import com.example.k2p.data.dto.LikesResponse
import com.example.k2p.domain.repository.LikeRepository
import javax.inject.Inject

class GetLikeByUserId @Inject constructor(private val likeRepository: LikeRepository) :
UseCase<LikesResponse, String>(){
    override suspend fun run(params: String) = likeRepository.getLikesByUserId(params)
}